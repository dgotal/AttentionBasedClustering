...
def train_EDESC(dataset, device, args, i, epochs=400):
    ...
    return (best_metrics['acc'], best_metrics['nmi'], best_metrics['kappa'],[], z_flat, acc_all, acc_selected, selected_band_indices)
...
def main(args):
    if args.mode == 'band_selection':
        ...
        rounds = 20 #Changed from 10 to 20
        cas = []
        ...
        for i in range(rounds):
            ...
            acc, nmi, kappa, ca, data_reshaped, acc_all, acc_selected, selected_band_indices = train_EDESC(dataset, device, args, i)
            acc_sum += acc
            nmi_sum += nmi
            kappa_sum += kappa
