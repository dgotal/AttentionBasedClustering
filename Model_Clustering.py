...
def train_EDESC(dataset, device, args, i, epochs=400):
    ...
    # --- CLUSTERING-BASED BAND SELECTION ---
    # 1. top-30 bands by attention value
    num_candidates = 30
    top_indices = np.argsort(attn.cpu().numpy())[::-1][:num_candidates]  # descending sort

    # 2. Izvuci spektralne vektore za te bandove
    data_all, labels = dataset.get_labeled_data()
    candidate_band_vectors = data_all[:, top_indices].T  # shape: [num_candidates, num_samples]

    # 3. KMeans clustering na spektralnim vektorima bandova
    num_final_bands = 10  # ili automatski
    kmeans = KMeans(n_clusters=num_final_bands, random_state=42)
    labels_kmeans = kmeans.fit_predict(candidate_band_vectors)

    # 4. Iz svakog klastera odaberi band s najvišom attention vrijednosti
    selected_band_indices = []
    for cluster_id in range(num_final_bands):
        cluster_members = np.where(labels_kmeans == cluster_id)[0]
        attn_values = attn[top_indices[cluster_members]].detach().cpu().numpy()
        best_in_cluster = cluster_members[np.argmax(attn_values)]
        selected_band_indices.append(top_indices[best_in_cluster])

    selected_band_indices = np.array(selected_band_indices)
    ...
    print(f"Selected bands in round {i}: {selected_band_indices}")
    print(f"Number of selected bands in round {i}: {len(selected_band_indices)}")

    tsne = TSNE(n_components=2).fit_transform(z_flat)
    plt.scatter(tsne[:,0], tsne[:,1], c=labels, cmap='tab20')
    plt.savefig(f'clusters_{i}.png')

    return best_metrics['acc'], best_metrics['nmi'], best_metrics['kappa'], acc_all, acc_selected, selected_band_indices
...
def main(args):
    if args.mode == 'band_selection':
        ...
        for i in range(rounds):
            ...
            acc, nmi, kappa, acc_all, acc_selected, selected_band_indices = train_EDESC(dataset, device, args, i)
            acc_sum += acc
            nmi_sum += nmi
            kappa_sum += kappa
            acc_all_list.append(acc_all)
            acc_selected_list.append(acc_selected)
            selected_bands_list.append(selected_band_indices)
            print(f"Selected bands in round {i}: {selected_band_indices}")
            print(f"Number of selected bands in round {i}: {len(selected_band_indices)}")
        ...
        print("average_acc (clustering):", acc_sum / rounds)
        print("average_acc (all bands):", np.mean(acc_all_list))
        print("average_acc (selected bands):", np.mean(acc_selected_list))
        ...
        for i, bands in enumerate(selected_bands_list):
            print(f"Round {i}: selected bands = {bands}, number = {len(bands)}")
